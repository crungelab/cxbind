...
class PbFunctionalRenderer(Renderer[T_Node]):
    def __init__(self, context: PbRenderContext, node: Method, object_type: ObjectType = None):
        super().__init__(context, node)
        self.object_type = object_type

    def render(self):
        method = self.node
        class_name = self.object_type.name.CamelCase() if self.object_type else ""
        ...
        for arg in args:
            ...
            if self.is_descriptor_member(arg):
                logger.debug(
