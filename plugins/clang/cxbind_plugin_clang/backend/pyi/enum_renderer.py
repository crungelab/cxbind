...
class PyiEnumRenderer(PyiNodeRenderer[EnumNode]):
    ...
    def render(self):
        ...
        convertible = not cursor.is_scoped_enum()

        # Same iteration and naming as the pb EnumRenderer.
        constants = [
            self.format_enum_constant(child.spelling, node.first_name)
        ...
        # .export_values() copies unscoped constants into the enclosing scope.
        # Declared once at the end of that scope; last registration wins.
        if convertible:
            for constant in constants:
                self.context.export(constant, path)