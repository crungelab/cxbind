...
class FunctionalBuilder(NodeBuilder[T_Node]):
    ...
    def build_argument_type(self, arg_cursor: cindex.Cursor, arg_spec: ArgSpec | None = None) -> Type:
        ...
        base_decl = self.get_base_declaration(arg_cursor.type)
    ...
    def build_return_value(self) -> None:
        ...
        self.node.returns = ReturnValue(type=ret_type, cursor=self.cursor.result_type, spec=spec)
    ...
    def build_return_type(self, return_spec: ReturnSpec | None = None) -> Type:
        ...
        base_decl = self.get_base_declaration(self.cursor.result_type)
        if base_decl is not None:
            base_key = Node.make_key(base_decl)
            logger.debug(f"Base key: {base_key}")
        else:
